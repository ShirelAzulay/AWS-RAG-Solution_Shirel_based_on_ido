...
def handle_run(record):
    ...
    logger.info(f"Processing: workspace={workspace_id}, docs={len(documents)}, provider={provider}, model={model_id}, query='{prompt[:50]}...'")

    if workspace_id and not documents:
        logger.warning(f"Empty documents for workspace query: {prompt}")
